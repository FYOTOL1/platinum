  ...
  return (
    <>
      <header className="lg:container flex items-center flex-row-reverse justify-between mx-auto py-3 px-2 overflow-x-scroll overflow-y-visible md:overflow-visible">
        ...
        />
        {/* Search Bar */}
        <div className="relative w-full min-w-[200px] max-w-[750px] h-[25px] sm:h-[35px] md:h-[45px] lg:h-[50px] sm:text-xs md:text-base">
          <label
            className="absolute right-4 md:right-6 top-[50%] translate-y-[-50%] text-[12px] sm:text-[14px]"
            htmlFor="search"
          >
          ...
          <input
            ...
            value={Search}
            className="pe-10 md:pe-14 ps-12 sm:ps-14 md:ps-16 h-full w-full text-end text-sm rounded-full [box-shadow:_0_2px_4px_rgba(0,0,0,.04),_0_0_4px_rgba(0,0,0,.06)] outline-none"
            type="text"
            placeholder="البحث عن الفعالية او الفئة"
          />
          {Search !== "" && (
            <label
              className="absolute left-3 sm:left-4 md:left-5 top-[50%] translate-y-[-55%] text-xs sm:text-sm md:text-base"
              htmlFor="search"
            >
              <button
                onClick={() => setSearch("")}
        ...
        {/* options */}
        <div className="flex items-center justify-between flex-row-reverse gap-2 sm:gap-4 md:gap-6 me-2 sm:me-4 md:me-6 lg:me-8 w-full">
          {/* Date */}
          <label className="text-lg md:text-xl lg:text-2xl" htmlFor="date">
          ...
          <button
            ...
            className="flex items-baseline flex-row-reverse gap-3 p-1 px-2 shadow-md outline outline-1 outline-gray-100 rounded-full text-[10px] sm:text-sm md:text-xl lg:text-2xl"
          >
            ...
            {/* Popup */}
            {IsAvatar ? (
              <div className="absolute mt-14 left-[111px] w-[350px] p-3 bg-white shadow-md outline outline-1 outline-gray-100 rounded-xl text-gray-600">
                <ul className="flex flex-col text-lg text-end">
                  <li
